...
<?php
	...
	if(isset($_POST['add_video'])){
		...
		if($vidata_qry){			
			...
			if($fire_vidmeta_qry){
					$explode_subject = explode(" ",$subjname);
					$subjname = implode("_",$explode_subject);
					$path_newdirectory = '../../'.$levelname.'/'.strtolower($subjname);				
					if(!is_dir($path_newdirectory)) {
					mkdir($path_newdirectory) or die('can not make new directory here <br />'.$path_newdirectory);					
					}
					$url_videopage = $path_newdirectory.'/'.$pageurl;
					...
					$vid_sourcefile = "../../beginners/raw_videofile.php";
