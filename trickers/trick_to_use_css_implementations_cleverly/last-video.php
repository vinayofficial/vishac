...
// Fetching previous video data
	...
	if ($selnextid = "SELECT * FROM vish_videodata WHERE vid_id>'$videoid' ORDER BY vid_id LIMIT 1" == 1){	
	$fire_selnextvid = mysqli_query($dbcon,$selnextid);
	$get_nextvid = mysqli_fetch_assoc($fire_selnextvid);
	$next_vid = SITE_PATH.$levelname.'/'.$subjname.'/'.$get_nextvid['vid_pageurl'];
	} else{	
			$fire_selnextvid = mysqli_query($dbcon,$selnextid);
	$get_nextvid = mysqli_fetch_assoc($fire_selnextvid);
			$next_vid = SITE_PATH.$levelname.'/'.$subjname.'/'.$get_nextvid['vid_pageurl'];
        ...
        <div class="video_navs">
            <div class="cmn-cntnr">
              ...
              <a id="facebookbtn-link" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=http:/<?php echo $_SERVER['PHP_SELF']; ?>">
                ...
                --><li><a href="<?php echo $next_vid ?>"><i class="fa fa-forward fa-lg"></i><span class="vidnav_lbl">Next</span></a></li>
                </ul>
            </div>
        </div>
        <div class="cmn-cntnr">
